...
If no mailbox is ready, admins can still create/reuse the contact and queue onboarding. The processor creates a native Power Pages invitation and writes the invitation code/redeem URL/expiry back to the request. If the site is private, first grant the user access in Power Pages Studio > Security > Site visibility. The admin can then provide the code/link verbally or through an approved CRDB communication channel.
...
- For private-site tests, confirm the target non-admin user appears in Site visibility > People who can access the site before invitation redemption.
...
- After redemption, confirm the invitation is no longer `New`, `adx_externalidentity` exists, the expected web role is present, and the assignment is active.
