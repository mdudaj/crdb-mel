...
def main() -> int:
    ...
    for alternate_key in schema.get("alternate_keys", []):
        operations.append({
            ...
            "target": f"{alternate_key['table']}.{alternate_key['name']}",
            "table": alternate_key["table"],
            "key_name": alternate_key["name"],
            "columns": alternate_key.get("columns", []),
            "web_api": "POST /api/data/v9.2/EntityDefinitions(...)/Keys",
            "solution_header": args.solution,
            "notes": alternate_key.get("notes", ""),
        })
    ...
    if args.json:
        ...
        if schema.get("alternate_keys"):
            print("\n## Alternate Keys")
            for alternate_key in schema["alternate_keys"]:
                columns = ", ".join(alternate_key.get("columns", []))
                print(f"- Create alternate key: {alternate_key['table']}.{alternate_key['name']} ({columns})")
