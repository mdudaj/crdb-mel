...
DATAVERSE_REQUEST_TIMEOUT_SECONDS = 180
...
class Dataverse:
    ...
    def request(self, method: str, path: str, *, payload: dict[str, Any] | None = None, solution_header: bool = False) -> requests.Response:
        ...
        response = self.session.request(method, f"{self.base}/{path.lstrip('/')}", json=payload, headers=headers, timeout=DATAVERSE_REQUEST_TIMEOUT_SECONDS)
...
def alternate_key_exists(dv: Dataverse, table_logical: str, key_schema: str) -> bool:
    response = dv.request(
        "GET",
        f"EntityDefinitions(LogicalName='{table_logical}')/Keys?$select=SchemaName&$filter=SchemaName eq '{key_schema}'&$top=1",
    )
    if response.status_code >= 400:
        raise RuntimeError(f"GET alternate key {table_logical}.{key_schema} failed: HTTP {response.status_code} {safe_error(response)}")
...
def create_table_payload(settings: Settings, table: dict[str, Any]) -> dict[str, Any]:
    ...
    ownership_type = table.get("ownership_type") or table.get("ownership") or "UserOwned"
    if ownership_type not in {"UserOwned", "OrganizationOwned"}:
        raise ValueError(f"Unsupported ownership type for {table_name}: {ownership_type}")
...
def alternate_key_schema_name(settings: Settings, alternate_key: dict[str, Any]) -> str:
    return schema_name(settings.publisher_prefix, alternate_key["name"])


def ensure_alternate_keys(dv: Dataverse, schema: dict[str, Any]) -> None:
    for alternate_key in schema.get("alternate_keys", []):
        table_name = alternate_key["table"]
        table = next((row for row in schema.get("tables", []) if row["name"] == table_name), None)
        table_logical = table_logical_name(dv.settings.publisher_prefix, table_name, table)
        key_schema = alternate_key_schema_name(dv.settings, alternate_key)
        if alternate_key_exists(dv, table_logical, key_schema):
            print(f"alternate key exists: {table_logical}.{key_schema}")
            continue
        payload = {
            "SchemaName": key_schema,
            "KeyAttributes": [column_logical_name(dv.settings.publisher_prefix, column) for column in alternate_key.get("columns", [])],
            "DisplayName": label(alternate_key["name"]),
        }
        dv.post(f"EntityDefinitions(LogicalName='{table_logical}')/Keys", payload, solution_header=True)
        print(f"alternate key created: {table_logical}.{key_schema}")
...
def print_summary(settings: Settings, schema: dict[str, Any], execute: bool) -> None:
    ...
    key_count = len(schema.get("alternate_keys", []))
    ...
    print(f"Alternate keys: {key_count}")
...
def main() -> int:
    ...
    ensure_alternate_keys(dv, schema)
