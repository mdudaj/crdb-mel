...
INVALID_XLSFORM_NAME_CHAR = re.compile(r"[^A-Za-z0-9_.-]")
OUTPUT_NAMESPACES = [
    ("h", "http://www.w3.org/1999/xhtml"),
...
def prepare_workbook(source: Path, output_dir: Path, form_title: str, version: str) -> tuple[Path, list[str]]:
    ...
    for row in range(2, survey.max_row + 1):
        ...
        if not name:
            continue
        original_name = name
        if not VALID_XLSFORM_NAME.match(name):
            name = INVALID_XLSFORM_NAME_CHAR.sub("_", name)
            if not name or not re.match(r"^[A-Za-z_]", name):
                name = f"_{name}"
            cell.value = name
            renamed[original_name] = name
            changes.append(f"survey row {row}: sanitized invalid name {original_name} -> {name}")
        seen[name] += 1
        if seen[name] == 1:
            continue
    ...
    if renamed:
        ...
        reference_col_indexes = [
            index + 1
            for header, index in survey_headers.items()
            if header in reference_columns
        ]
        for row in range(2, survey.max_row + 1):
            for col in reference_col_indexes:
                cell = survey.cell(row=row, column=col)
                ...
                if updated != cell.value:
                    cell.value = updated
                    changes.append(f"survey row {row}: updated renamed field reference in {survey.cell(row=1, column=col).value}")
