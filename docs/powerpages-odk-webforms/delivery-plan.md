...
- Reporting/export implementation follows `reporting-export-delivery-plan.md` before any Power BI-facing feature is built.
...
## Planned Slice: Reporting, Export, and Power BI

Implementation instructions:

1. Inspect `reporting-export-research.md`, `reporting-export-requirements.md`, `adr-0003-reporting-export-powerbi.md`, `reporting-export-delivery-plan.md`, `schemas/dataverse/odk-central-inspired-mvp-schema.json`, and the current submit/edit code before drafting schema.
2. Treat `Submissions`, `SubmissionVersions`, XML, and JSON payloads as canonical. Reporting rows are derived and rebuildable.
3. Draft additive Dataverse schema artifacts for root report rows, repeat rows, answer rows, and named export settings before live writes.
4. Do not implement Power BI over developer-generated CSV files. The primary path is the Dataverse connector reading governed reporting tables.
5. Keep CSV/XLSX download as user-facing export UX, with XLSX required when repeat groups must be included.
6. Use Power Pages `/_api` and Dataverse permissions for portal data views; do not expose anonymous export links or credentials.
7. Verify schema dry-run, projection rebuild, submit/edit projection refresh, Data UX browser behavior, CSV/XLSX downloads, and Power BI Desktop connection with an authorized organizational account.
