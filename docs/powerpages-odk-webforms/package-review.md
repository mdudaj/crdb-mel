...
- Verify uploaded SPA assets by Power Pages `adx_partialurl` and content hash, not by downloaded local filename alone. PAC can export duplicate web-file records with suffixed local filenames when multiple records share the same partial URL. Use `node scripts/verify-powerpages-spa-assets.mjs` before upload and after a fresh post-upload download. On CRDB in August 2026, the three ODK locale chunks were exported with duplicate records, but runtime safety depended on at least one record for each expected browser URL having the exact current binary.
- For CRDB specifically, apply `crdb-deployment-asset-verification-rule-20260820.md`: fail on missing/mismatched current assets or stale Home references; record known managed duplicate residue as server hygiene unless it breaks current asset safety.
